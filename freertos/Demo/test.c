...
{
    if (testConfiguration == NULL) return false;
    if (testConfiguration->num_tasks > 0 && testConfiguration->tasks == NULL) return false;
    if (testConfiguration->num_tasks > testConfiguration->max_tasks) return false;

    for (uint32_t i = 0; i < testConfiguration->num_tasks; i++)
    {
        TaskConfiguration_t currentTask = testConfiguration->tasks[i];

        if (currentTask.func == NULL) return false;
        ...
        vCreateAndAddTask(
            ...
            ((currentTask.deadline == 0) && (currentTask.period == 0)) ? false : true, // Auto-detect Periodic
